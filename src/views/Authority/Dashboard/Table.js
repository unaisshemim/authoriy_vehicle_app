  ...
  const getData = async () => {
    try {
      const response = await axios.get(
        "https://oman-bc-02381b34ea2d.herokuapp.com/transaction"
      );
      setData(response.data.result.transfers);
    } catch (error) {
      console.log(error.message);
    }
  };
  if (data.length > 0) {
    for (const originalObject of data) {
      const extractedData = {
        id: originalObject.hash,
        value: originalObject.value,
        block: originalObject.blockNum,
        from: originalObject.from,
        to: originalObject.to,
      };
      filtered.push(extractedData);
    }
  }
